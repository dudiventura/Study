...
public class Ball 
{
	...
	private boolean bounceRacket, onRacket;
	...
	public void drawBall(Graphics2D dbg)
	{
		dbg.drawImage(Settings.BALL_IMG, locX, locY, size, size, null);
	}
	...
	private void bounceAtRacket()
	{
		...
		if(ballBox.intersects(racketBox))
		{
			String fileName = (new File(".")).getAbsolutePath() + "\\poing.wav";
	...
	public void bounceAtBrick(Brick b)
	{
		...
		{
			String fileName = (new File(".")).getAbsolutePath() + "\\brick_bounce.wav";
	        (new SoundThread(fileName, AudioPlayer.ONCE)).start();
